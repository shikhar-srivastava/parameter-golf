...
class Hyperparameters:
    ...
    # Replaces per-block (attn_scale, mlp_scale, ln_scale_factor) with a depth-conditioned
    # gate computed by DepthGates. For each step k (with u = log(k+1)) and each gate
    # position p in {input_attn, input_mlp, residual_attn, residual_mlp}:
    #     mag_p(k)   = α_p + β_p * u
    #     Θ_p(k)     = exp(α_rot,p + β_rot,p * u)
    #     θ_{p,j}(k) = Θ_p(k) * base^(-2j/H)            j ∈ [0, H/2)
    # γ_eff,p(k) is then the complex rotation of γ_p (treating consecutive pairs as
    # Re+i·Im) by exp(mag_p(k) + i·θ_{p,j}(k)), all FP32. See class DepthGates.
    #
    # Indexing mode:
    #   "exec_step"  (default) — k is the position in the unrolled forward sequence
    #                            (0..max_steps-1); aligns with how BatchedTTTLoRA
    #                            already keys its per-visit adapters.
    #   "physical"             — k is the physical layer index (0..num_layers-1);
    #                            looped layers reuse the same gate at every visit
    #                            (matches the published shared-gamma convention).
    gate_alpha_init = float(os.environ.get("GATE_ALPHA_INIT", 0.0))
    gate_beta_init = float(os.environ.get("GATE_BETA_INIT", -0.5))
    gate_alpha_rot_init = float(os.environ.get("GATE_ALPHA_ROT_INIT", 0.0))
    gate_beta_rot_init = float(os.environ.get("GATE_BETA_ROT_INIT", -0.5))
    gate_rope_base_freq = float(os.environ.get("GATE_ROPE_BASE_FREQ", 100.0))
    depth_gate_indexing = os.environ.get("DEPTH_GATE_INDEXING", "exec_step")
...
class DepthGates(nn.Module):
    """LayerRoPE: complex-rotation depth gates with shared gamma and per-pair RoPE-style frequency.

    Replaces the baseline's per-block (attn_scale, mlp_scale, ln_scale_factor) with a
    depth-conditioned gate computed once per visit and applied uniformly at four positions:
    {input_attn, input_mlp, residual_attn, residual_mlp}. For each visit step k (with
    u = log(k+1)) and each gate position p:

        mag_p(k)   = α_p + β_p · u                              [FP32 scalar]
        Θ_p(k)     = exp(α_rot,p + β_rot,p · u)                  [FP32 scalar]
        θ_{p,j}(k) = Θ_p(k) · base^(−2j/H), j ∈ {0..H/2-1}       [FP32 (H/2,)]

    The effective γ at step k is obtained by treating consecutive pairs (γ_p[2j], γ_p[2j+1])
    as Re+i·Im and multiplying by exp(mag_p(k) + i·θ_{p,j}(k)):

        γ_eff,p[2j]   = e^{mag_p(k)} · ( γ_p[2j]·cos(θ_{p,j}) − γ_p[2j+1]·sin(θ_{p,j}) )
        γ_eff,p[2j+1] = e^{mag_p(k)} · ( γ_p[2j]·sin(θ_{p,j}) + γ_p[2j+1]·cos(θ_{p,j}) )

    γ_eff,input_branch is applied as a per-channel multiplier on the pre-norm output
    (replaces ln_scale_factor); γ_eff,residual_branch is applied on the sub-block output
    (replaces per-block attn_scale/mlp_scale).

    Indexing:
      The caller passes `step_idx` to `forward(step_idx)`. With DEPTH_GATE_INDEXING="exec_step",
      step_idx is the position in the unrolled forward sequence (0..max_steps-1) and looped
      layers receive PROGRESSIVELY MORE damping at each visit. With "physical", step_idx is
      the physical layer index (0..num_layers-1) and looped layers reuse the same gate.

    All gate arithmetic runs in FP32 to match the FP32 invariants of the surrounding stack
    (Muon updates, GPTQ Hessians, EMA). The two precomputed buffers (log_depths, rope_freqs)
    are FP32 and persistent=False so they stay out of state_dict; restore_fp32_params re-casts
    them after the global .bfloat16() at model build.
    """
    POS = ("input_attn", "input_mlp", "residual_attn", "residual_mlp")
    ...
    def __init__(
        ...
        alpha_rot_init=0.0,
        beta_rot_init=-0.5,
        rope_base_freq=100.0,
    ):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.max_steps = int(max_steps)
        self.rope_base_freq = float(rope_base_freq)
        for pos in self.POS:
            self.register_parameter(
                f"gamma_{pos}",
                nn.Parameter(torch.ones(hidden_dim, dtype=torch.float32)),
            )
            self.register_parameter(
                f"gate_alpha_{pos}",
                nn.Parameter(torch.tensor(float(alpha_init), dtype=torch.float32)),
            )
            self.register_parameter(
                f"gate_beta_{pos}",
                nn.Parameter(torch.tensor(float(beta_init), dtype=torch.float32)),
            )
            self.register_parameter(
                f"gate_alpha_rot_{pos}",
                nn.Parameter(torch.tensor(float(alpha_rot_init), dtype=torch.float32)),
            )
            self.register_parameter(
                f"gate_beta_rot_{pos}",
                nn.Parameter(torch.tensor(float(beta_rot_init), dtype=torch.float32)),
            )
        # Precomputed FP32 constants. persistent=False -> excluded from state_dict
        # (deterministic from constructor args). .bfloat16() at model build downcasts these;
        # restore_fp32_params re-casts back to FP32 (see DepthGates branch there).
        self.register_buffer(
            "log_depths",
            torch.tensor(
                [math.log(k + 1.0) for k in range(self.max_steps)],
                dtype=torch.float32,
            ),
            persistent=False,
        )
        j = torch.arange(hidden_dim // 2, dtype=torch.float32)
        self.register_buffer(
            "rope_freqs",
            float(rope_base_freq) ** (-2.0 * j / hidden_dim),
            persistent=False,
        )

    @staticmethod
    def _rotate(gamma, mag, theta):
        """Apply exp(mag + i·theta) pairwise to gamma.
        gamma: (H,) FP32. mag: scalar FP32. theta: (H/2,) FP32. Returns (H,) FP32."""
        g_re = gamma[0::2]
        g_im = gamma[1::2]
        R = torch.exp(mag)
        c = torch.cos(theta)
        s = torch.sin(theta)
        u_out = R * (g_re * c - g_im * s)
        v_out = R * (g_re * s + g_im * c)
        return torch.stack([u_out, v_out], dim=-1).flatten(-2)

    def _gate(self, gamma, a, b, ar, br, log_l, rope_freqs):
        mag = a + b * log_l
        theta = torch.exp(ar + br * log_l) * rope_freqs
        return self._rotate(gamma.float(), mag, theta)

    def forward(self, step_idx):
        """Return (γ_eff_input_attn, γ_eff_input_mlp, γ_eff_residual_attn, γ_eff_residual_mlp), each (H,) FP32."""
        log_l = self.log_depths[step_idx]
...
class Block(nn.Module):
    def __init__(
        ...
        # ln_scale_factor and per-block (attn_scale, mlp_scale) replaced by LayerRoPE
        # DepthGates. The gates are passed in via forward(...) as a 4-tuple of (H,) FP32
        # tensors: (γ_eff_input_attn, γ_eff_input_mlp, γ_eff_residual_attn, γ_eff_residual_mlp).
        # Input gates multiply the pre-norm output; residual gates multiply sub-block out.

    def forward(self, x, x0, q_w, k_w, v_w, out_w, up_w, down_w, gates, cu_seqlens=None, max_seqlen=0):
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        # ===== LayerRoPE depth gates =====
        # max_steps must cover both the no-loop and loop-active execution lengths.
        # In exec_step mode the gate at index k uses log(k+1); we precompute log_depths
        # for max(num_layers, len(encoder_indices)+len(decoder_indices)) entries so the
        # buffer is sized for the larger (looping) case. Indices > num_layers-1 only
        # fire after looping_active flips at frac=enable_looping_at, which is fine —
        # they extrapolate the same (α, β, α_rot, β_rot) formula smoothly.
        max_loop_steps = len(self.encoder_indices) + len(self.decoder_indices)
        max_steps = max(h.num_layers, max_loop_steps)
        self.depth_gate_indexing = str(getattr(h, "depth_gate_indexing", "exec_step"))
        if self.depth_gate_indexing not in ("exec_step", "physical"):
            raise ValueError(
                f"DEPTH_GATE_INDEXING must be 'exec_step' or 'physical', got {self.depth_gate_indexing!r}"
            )
        self.depth_gates = DepthGates(
            h.model_dim,
            max_steps,
            alpha_init=h.gate_alpha_init,
            beta_init=h.gate_beta_init,
            alpha_rot_init=h.gate_alpha_rot_init,
            beta_rot_init=h.gate_beta_rot_init,
            rope_base_freq=h.gate_rope_base_freq,
        )
        self._init_weights()
    ...
    def _parallel_block(
        ...
        q_w, k_w, v_w, out_w, up_w, down_w, gates,
    ...
    def _forward_hidden(self, input_ids, cu_seqlens=None, max_seqlen=0):
        ...
        # ===== LayerRoPE: compute gates per visit =====
        # In exec_step indexing, gate at visit position k uses log_depths[k] (the
        ...
        for i in enc_iter:
            ...
            x = self.blocks[i](x, x0, q_w, k_w, v_w, out_w, up_w, down_w, gates_per_step[step], cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)
            step += 1
            skips.append(x)
        ...
        for skip_idx, i in enumerate(dec_iter):
            ...
            if i >= psl and psl > 0:
                ...
                lane0, lane1 = self._parallel_block(
                    i, lane0, lane1, x0, q_w, k_w, v_w, out_w, up_w, down_w, gates_per_step[step],
                    cu_seqlens=cu_seqlens, max_seqlen=max_seqlen,
                )
                step += 1
            else:
    ...
    def forward_ttt(self, input_ids, target_ids, lora):
        ...
        for i in enc_iter:
            q_w, k_w, v_w, out_w, up_w, down_w = self._bank_weights(i)
            x = self._block_with_lora(self.blocks[i], x, x0, lora, slot, q_w, k_w, v_w, out_w, up_w, down_w, gates_per_step[slot])
            slot += 1
            skips.append(x)
        psl = self.parallel_start_layer
        lane0 = None
        ...
        for skip_idx, i in enumerate(dec_iter):
            ...
            if i >= psl and psl > 0:
                ...
                lane0, lane1 = self._parallel_block_with_lora(
                    ...
                    q_w, k_w, v_w, out_w, up_w, down_w, gates_per_step[slot],
                )
            else:
                if skip_idx < self.num_skip_weights and skips:
                    scaled_skip = (
    ...
    def _block_with_lora(self, block, x, x0, lora, slot, q_w, k_w, v_w, out_w, up_w, down_w, gates):
        g_in_attn, g_in_mlp, g_res_attn, g_res_mlp = gates
        ...
        x_out = x_out + g_res_mlp.to(dtype=x_out.dtype)[None, None, :] * mlp_out
        return x_out
    ...
    def _parallel_block_with_lora(
        ...
        if lora.mlp_loras is not None:
            mlp_out = mlp_out + lora.mlp_loras[slot](mlp_n)
        mlp_out = g_res_mlp.to(dtype=lane1.dtype)[None, None, :] * mlp_out
        attn_resid = self.parallel_resid_lambdas[block_idx, 0].to(dtype=lane0.dtype)
        attn_post = self.parallel_post_lambdas[block_idx, 0].to(dtype=lane0.dtype)
        mlp_resid = self.parallel_resid_lambdas[block_idx, 1].to(dtype=lane0.dtype)
        mlp_post = self.parallel_post_lambdas[block_idx, 1].to(dtype=lane0.dtype)
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gates,parallel_post_lambdas,parallel_resid_lambdas,attn_gate_proj,attn_gate_w,smear_gate,smear_lambda,gate_alpha,gate_beta,gamma_input,gamma_residual",
    ).split(",")
    if pattern
)
...
def restore_fp32_params(model):
    for module in model.modules():
        if isinstance(module, CastedLinear):
            module.float()
        # LayerRoPE: log_depths and rope_freqs are FP32 by construction but
        # downcast by the global .bfloat16() at model build. Recast here.
        if isinstance(module, DepthGates):
            module.log_depths = module.log_depths.float()
            module.rope_freqs = module.rope_freqs.float()
